...
public abstract class LibALFObject {
	 ...
	 */
	transient int id;
	transient DispatcherSession session;

	protected LibALFObject(DispatcherSession session, ObjectType ot) {
		this.id = (this.session = session).dispatchCreateObject(ot);
		if (this.id < 0)
			throw new AlfException("Creating Object " + ot + " failed");
	}
