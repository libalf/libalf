...
class DFAminiSAT : public learning_algorithm<answer> {
	...
	public:
		...
		/**
		 * Creates a SAT instance for the given data.
		 *
		 * Note: The parameters x, d, z should be an uninitialized pointer
		 *       because this method will initialize it.
		 */
		bool create_sat(Solver & solver, unsigned int state_count, std::vector<std::vector<Var> > & x, std::vector<std::vector<std::vector<Var> > > & d, std::vector<Var> & z) {
			...
			for(unsigned int w=0; w<tree.size(); w++) {
					...
					if(tree[w][a] != NO_EDGE) {
						...
						for(unsigned int p=0; p<state_count; p++) {
							for(unsigned int q=0; q<state_count; q++) {
								...
								#endif
								solver.addTernary(Lit(d[p][a][q]), ~Lit(x[w][p]), ~Lit(x[tree[w][a]][q]));
								if(!solver.okay()) {
		...
		/**
		 * Derives a DFA from the model computed.
		 */
		inline finite_automaton * sat2automaton(Solver & s, unsigned int state_count, std::vector<std::vector<Var> > & x, std::vector<std::vector<std::vector<Var> > > & d, std::vector<Var> & z) {
