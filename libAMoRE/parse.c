...
static char empty_buf[] = "Empty input buffer",
    ...
    no_val_in[] = "No valid input buffer", no_ex_b_p[] = "No expression between parentheses";
...
{
	symbol_stack outstack = (symbol_stack) newbuf(1, sizeof(struct str_symbol_stack));
...
static GRAMMAR read_grammar(posint n, char startsym, array_of_c_string strar)
{
	...
	if(n <= 0)
		return NULL;	/* No rules */
	do {
		s = strar[sc];
		sl = strlen(s);
		j = 0;
		while(((s[j] != '-') || (s[j + 1] != '>')) && (j < (sl - 2)))
			++j;
		/* Look for -> */
		if(j == (sl - 2))
		...
		do {		/* Look for right sides for t */
			...
			++i;
			if(s[j] == '|')
				++j;	/* Look for further right sides */
		} while(s[j] != '\0');
		++sc;
	} while(i < n);
...
posint *lno;
{
	...
	while(l->info[p] == spacech) {
		if(p == l->lastpos) {	/* End of textline reached */
			...
			++p;	/* Get next character on actual line */
	}
...
posint *lno;
{
	...
	if(p < l->lastpos)
		++p;		/* End of textline not yet reached */
	else if(l->next != NULL) {	/* End of textline: if a next line exists */
...
GRAMMAR gr;
{
	...
	posint z, maxp, maxlno, maxz = gr->maxidlen + 1, hp = (*pos), hlno = (*lno);
...
GRAMMAR gr;
{
	...
	} else {		/* It's not an operator, thus it's an */
		/* identifier */
...
array_of_int *precedence;
{
	...
	posint synchr, r, i, j, errorpos, nonterm,
	    lenhandle = 1, nextindex = index[(int) (*top_symbol)->op], preindex = index[(int) pretype];
	REDUCE_RESULT outres = newbuf_reduce_result();
	t_elem hlp;

	while(precedence[preindex][nextindex] == EQUAL) {	/* Get the size of the handle */
		++lenhandle;
		nextindex = preindex;
	...
	for (r = 0; r < gr->ruleno; ++r) {	/* Look for a rule that contains all symbols */
		/* of the handle */
		for (i = 0; i < lenhandle; ++i)
			if(!gr->isinrule[index[(int) handle[i]]][r])
				break;
		...
		if(i == lenhandle) {	/* We have found such a rule */
			...
			while(rightside[i] != rightmost->op)
				--i;
			s_elem = (*top_sentence);
			while(!((s_elem->op == rightmost->op) &&
				(s_elem->position == rightmost->position) && (s_elem->line == rightmost->line)))
				s_elem = s_elem->prev;
			...
			for (j = i; j > 0; --j, down = down->prev)
				if(down->op != rightside[j - 1])
					break;
			down = down->next;
			if(j > 0) {	/* The downward comparison failed */
				errorpos = j - 1;
				synchr = i;
				continue;
			}
			...
			j = i;
			up = s_elem;
			while((up->next != NULL) && (j < strlen(rightside)) && (up->op == rightside[j])) {
				++j;
				up = up->next;
			}
			if(up->op != rightside[j]) {	/* The upward comparison failed */
				errorpos = j;
				synchr = i;
				continue;
			}
			if(j < (strlen(rightside) - (size_t) 1)) {	/* Only a real prefix of the rule was found */
				errorpos = j + 1;
				synchr = i;
				continue;
			}
	...
	if(r == gr->ruleno) {	/* No rule contains all symbols of *//* the handle */
	...
	for (i = 0, s_elem = down; i < nonterm; s_elem = s_elem->next)
		if(!isoperator(s_elem->op, gr->operator_list, gr->maxop)) {	/* Each nonterminal represents */
...
sentence_stack *top_sentence;
{
	...
	for (i = 0; i < strlen(elem->symbols); ++i)
		s[i] = elem->symbols[i];
	(*top_symbol)->next = new_symbol_stack(elem->type, elem->position, elem->line);
	((*top_symbol)->next)->prev = (*top_symbol);
	(*top_symbol) = (*top_symbol)->next;

	(*top_sentence)->next = new_sentence_stack(elem->type, s, elem->position, elem->line);
...
static void red_err(inres, inred_res, top_sentence)
PARSE_RESULT inres;
...
sentence_stack top_sentence;
{
	...
	if(inred_res->rule[inred_res->synchr] != rightbrch) {
		outmessage = (inred_res->errorpos < inred_res->synchr) ? miss_op_b : miss_op_a;
		errorsymbol = s_elem->op;
	} else {
		outmessage = no_ex_b_p;
		errorsymbol = spacech;
	}
	inres->error_message = getmessage(outmessage, errorsymbol, s_elem->position + 1, s_elem->line + 1);
...
posint prec;
{
	...
	switch (prec) {
	case UNBALBR:
		errormessage = unba_lebr;	/* Unbalanced left bracket */
		inres->error_message = getmessage(errormessage, spacech, topsymbol->position + 1, topsymbol->line + 1);
		inres->error_pos = topsymbol->position;
		inres->error_line = topsymbol->line;
		break;

	case UNBARBR:
		errormessage = unba_ribr;	/* Unbalanced left bracket */
		inres->error_message = getmessage(errormessage, spacech, elem->position + 1, elem->line + 1);
		inres->error_pos = elem->position;
		inres->error_line = elem->line;
		break;

	case ENDTEXT:
		break;		/* End of the input buffer */
...
static PARSE_RESULT parse(strstack inbuf, GRAMMAR gr, array_of_int index, array_of_int * precedence)
{
	...
	posint prec, lastpos, lno = 0, pos = 0;
	strstack endline, line = inbuf;
	tbuf_elem elem;
	char lasttype;
	boole endparse = FALSE, reduction = FALSE;
	symbol_stack top_symbol;
	sentence_stack top_sentence;
	...
	do {
		if(!reduction)
		{		/* If a reduction was done, the last */
			...
			if(elem->symbols == NULL) {	/* An unexpected symbol ocurred */
				...
				outres->error_message = (elem->type == endch) ? un_ex_eob :
				    getmessage(unexp_sym, elem->type, pos + 1, lno + 1);
		...
		switch (prec) {
		case GREATER:
			red_res = reduce(&top_symbol, &top_sentence, gr, index, precedence);
			if(red_res == NULL)
				reduction = TRUE;
			/* reduction must be set to TRUE, because */
			/* the old symbol must be processed again */
			else {	/* The reduction failed, red_err produces */
				/* the errormessage */
		...
		case EQUAL:;
		case LESS:
			shift(elem, &top_symbol, &top_sentence);
			reduction = FALSE;
		...
		default:
			prec_diag(outres, top_symbol, elem, prec);
...
PARSE_RESULT parser(PARSE_INPUT instruct)
{
	...
	gr = init_grammar(instruct->rules, instruct->startsymbol, instruct->maxrule);
	gr->maxidlen = 0;
	gr->ident_list = init_ident_list(instruct->id_list, &(gr->maxid), &(gr->maxidlen));
	gr->prefix_list = init_prefix_list(gr->ident_list, gr->maxid, &(gr->maxpref));
	...
	if(res == NULL)
		res = parse(instruct->buf, gr, instruct->index, instruct->precedence);
	else
		res->error_pos = (res->error_line = 0);
