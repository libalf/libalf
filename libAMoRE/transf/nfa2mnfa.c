...
#include <amore/nfa2dfa.h>	/* modnfa2dfa */
#include <amore/dfa2nfa.h>	/* dfa2nfa */
...
nfa nfa2mnfa(nfa na, dfa minda)
{
	...
	if(V) {			/* The calculation is based on the rows and on the       */
	...
	} else {		/* The calculation is based on the columns and on the    */
...
static boole row_is_covered(posint j, posint k)
{
	...
	if(rowweight[j] >= rowweight[k])
		return FALSE;
	for (i = 0; i <= SPALTEN; i++)
		if(testelem(j, tra[i]) && (!testelem(k, tra[i])))
			return FALSE;
	return TRUE;
}
...
static boole col_is_covered(posint j, posint k)
{
	...
	if(colweight[j] == 0)
		return FALSE;	/* Empty state. */
	if(colweight[j] >= colweight[k])
		return FALSE;
	for (i = 0; i <= ZEILEN; i++)
		if(testelem(i, tra[j]) && (!testelem(i, tra[k])))
			return FALSE;
	return TRUE;
}
...
static void compute_compact_rows()
{
	...
	for (i = 0; i <= ZEILEN; i++) {
		if(rowweight[i] < 2) {
			if(rowweight[i] == 0) {	/* If there is no 1 in the row, the row can be       */
			...
			for (j = 0; j <= ZEILEN; j++)
				if(j != i)
					if(row_is_covered(j, i)) {	/* Found a row that contains at least one 1 and  */
						/* is covered by row i.                          */
			...
			if(pattern->ngrset == 0) {	/* Found a row that is the union of some other rows. */
				add(i, compact_row->grset);
				compact_row->ngrset++;
			}
		}
...
static void compute_compact_cols()
{
	...
	for (i = 0; i <= SPALTEN; i++) {
		if(colweight[i] < 2) {
			if(colweight[i] == 0) {	/* If there is no 1 in the column, the column can be */
			...
			for (j = 0; j <= SPALTEN; j++) {
				if(j != i) {
					if(col_is_covered(j, i)) {	/* Found a column that contains at least one 1   */
			...
			if(pattern->ngrset == 0) {	/* Found a column that is the union of some other    */
				/* columns.                                          */
				add(i, compact_col->grset);
				compact_col->ngrset++;
			}
			  ...
			  /* and of irreducible automaton.   */
{
	...
	for (j = 0, k = covered_states[i]->ngrset; k != 0; j++) {
		if(testelem(j, covered_states[i]->grset)) {
			k--;
			if(!testelem(j, compact_states->grset)) {	/* Found another initial state. */
	...
	for (i = 0; i <= dastates; i++) {
		if(!testelem(i, compact_states->grset)) {	/* Only for states that are not compact. */
			for (j = 1; j <= da->alphabet_size; j++) {
				...
				} else {	/* K is not compact. Simply join old2new[k] to     */
					/* delta[j][i].                                    */
...
static void calculate_necessary_grids()
{
	...
	if(V) {			/* Calculate with rows. First try to find a row with     */
		/* rowweight==1.                                         */
		for (i = 0; i <= ZEILEN; i++) {
			if(rowweight[i] == 1) {	/* Found such a row. Now look for the column. */
				test = FALSE;
				j = 0;
				...
				if((test) && (colweight[j] == 1)) {	/* The 1 stands alone in row i and column j. Now   */
					/* define a new grid.                              */
					ones->ngrset++;
					add(i, ones->grset);	/* Store the number of the row. */
					if(grids->sizeofglist != 0) {	/* A new gridlist is needed. */
	...
	} else {		/* Calculate with columns. First try to find a column    */
		/* with colweight==1.                                    */
		for (i = 0; i <= SPALTEN; i++) {
			if(colweight[i] == 1) {	/* Found such a column. Now look for the row. */
				test = FALSE;
				j = 0;
				while((!test) && (j <= ZEILEN)) {
					if(testelem(j, tra[i]))
				...
				if((test) && (rowweight[j] == 1)) {	/* The 1 stands alone in row j and column i. Now   */
					/* define a new grid.                              */
					ones->ngrset++;
...
static void calculate_grids_r()
{
	...
	k = 0;
	add(k, pattern->grset);
	if(easy) {		/* Make a grid out of every pattern. */
		while(k <= Z) {
		...
		while(k <= Z) {	/* Test if there is a column defining a grid together  */
			...
			if(testI) {
				add_to_list(pattern, new2old);
				k = 0;
			} else {	/* If no such column was found not all of the        */
...
static void calculate_grids_c()
{
	...
	if(easy) {		/* Make a grid out of every pattern. */
		...
		while(k <= S) {	/* Test if there is a row defining a grid together     */
			...
			if(testI) {
				add_to_list(pattern, new2old);
				k = 0;
			} else {	/* If no such row was found not all of the remaining */
	       ...
	       /* of the original RAM.                       */
{
	...
	if(grids->sizeofglist != 0) {	/* A new gridlist has to be defined. */
		newgridlist(grids->glist->next, nbs);
		grids->glist = grids->glist->next;
	}
	if(REST == 0)
	...
	for (j = 0; i != 0; j++) {
		if(testelem(j, pattern->grset)) {	/* Found a column. */
			i--;
