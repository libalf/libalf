...
{ nfa result=NULL;
  ...
  if(minimal==(MAXI+1-NECESSARY))
    ...
    {/* The automaton has to be calculated. */
     result=calculate_d(delta,i_gr,f_gr,fI,f); 
     if(version) /* Otherwise dual language.   */
       {freebuf();return (nfa) revnfa(result);}
     else
       {freebuf();return result;}
    }
}
...
{ gridlist check;
  ...
  for (i=0; i<=f->qno; i++)
    if (isfinal(f->infin[i]))
    { /* I is an initial state of the dual automaton to f.   */
      ...
      if (!test)
      { /* A new term has to be defined. Check->info is the  */ 
        ...
        for (/* j=NECESSARY,check=fII */; j<GITTER; j++,check=check->next)
          if((testelem(j,i_gr->grset)) && (testelem(i,check->info->col)))
            {/* Found a new variable for the sum. */
...
{ posint i,j,k,l,s,t;
  ...
  for (i=0;(i<GITTER)&&go_on; i++)
    if ((i==NECESSARY)&&(number==0)) go_on=FALSE; 
      ...
      for (j=0; j<le; j++)
        if((t=delta[j][i]->ngrset)!=0)
          {/* There are grids that can be covered. */
           for (l=0,check=fI,s=0; l!=GITTER; l++,check=check->next)
             {if(grid_is_covered(delta,check->info,j,i))
                { /* Found a grid whose columns are  */
           ...
           if(s!=0)
            ...
            for (k=0; t!=0; k++)
               if(testelem(k,delta[j][i]->grset))
                 {t--;
                  for (l=0,check=fI,test=FALSE; ((!test) && (l<NECESSARY)); l++,check=check->next)
                    if (testelem(l,helpII->grset))
                      if (testelem(k,check->info->col)) test=TRUE; 
                        /* No term has to be defined, because  */
                        /* an essential grid for which the     */
                        /* above condition holds is found.     */
                  if (!test) define_term(check,helpII,i,k,s);     
...
{ posint l,m,variable;
  ...
  if (i>=NECESSARY) help_term->k=i-NECESSARY;        
  else              help_term->k=GITTER;                                
...
{ posint count,variable,oldnumber;
  ...
  if (help_term->k==GITTER) 
  { /* The current term has no complemented variable. So far */
    ...
    { /* Compare term_array with run->info. */
      if(nv<run->nv) 
    ...
    if (!found) 
    { /* Term_array is a new term and has to be added to the */
      /* list.                                               */
      if (number==0)
  ...
  { /* Since help->k!=GITTER the currently worked with term  */
    /* has a complemented variable.                          */
    if (help_term->k!=actcomplemented) 
    { /* The term has a new complemented variable. */
      if (actcomplemented==GITTER)
      { /* No complemented variable so far. */
    ...
    if (!found)
    { /* Compare term_array with the other terms with the    */
      /* same complemented variable.                         */
      if (actcount==0)
      { /* No other term with the same complemented variable */
      ...
      { /* There are some other terms with the same          */
        ...
        if (!found)
        { /* The term term_array is unknown and added to the */
          /* list.                                           */
          append_term();
          actcount++;
        }
...
{if(number!=1)
   {if(run->next==NULL) /* Run is the last element.  */
      {last=last->bef;last->next=NULL;}
    else if(run->bef==NULL) /* Run is the first element. */
      {first=first->next; first->bef=NULL;}
    else /* Run has a termlist before and after it. */
      {run->next->bef=run->bef;run->bef->next=run->next;}  
   }
...
{ posint count;      
  ...
  for (count=1;count<=number;count++,run=run->next)
    {NV[count]=run->nv;T[count]=run->info;}                          
  /* The array start has to be altered. */
  if(actcomplemented==GITTER) start[0]=number+1;  
...
{ posint variable,i;
  ...
  for(i=1;i<start[0];i++)nextlevel[i]=i; 
  maxlevel=start[0];
  newpattern(current_best,nbr);
  while (actlevel>0) 
  /* Look at the current term and alter the partial        */ 
  /* solution if necessary.                                */ 
    {if (!find_next_partial_solution()) return (current_best);
	...
	{/* Found a new best so far solution. Now save it. */
        minimal=current_best->ngrset=novar;        
        for (variable=0;variable<REST;variable++)
          if(startlevel[variable]){add(variable,current_best->grset);}
          else                    {sub(variable,current_best->grset);}
        if (minimal==1)
          return current_best; /* Nothing better to find. */
        else
          {/* Start searching for another solution. Therefore  */
           /* remove one variable from the solution and find   */
           /* the term the second-from-last one came from.     */
           while(collection[actlevel]==0) actlevel--;
       ...
       {/* There are still terms variables have to be taken    */
        ...
        if((minimal-1)==novar)
          {/* Try to find a solution by not adding another     */
           /* (uncomplemented) variable to the solution.       */
           if(speed_up())
             {/* Found a new best so far solution. Save it.    */
              minimal=current_best->ngrset=novar;         
              for(variable=0;variable<REST;variable++)
              ...
              if (minimal==1) return current_best; 
              else
...
/* This is a dirty trick added by oma on 20.09.96 */
static boole find_next_partial_solution()
{ boole notfound=TRUE; posint position,variable,level;  
  ...
  if(collection[actlevel]==0)
    {/* No variable has been taken from the term. */
     if (!cut_is_empty(level))actlevel++; 
       /* No new variable has to be added to the  */
       /* list. Simply go to the next level.      */
     else
       {position=0;
        while ((position<NV[level])&&(notfound))
          if(!lock[T[level][position]])notfound=FALSE; 
        ...
        if(notfound) 
	  {
	    ...
	    /* this was added by oma, 20.09.96 */
          ...
          {/* Add T[level][position] to the partial solution.   */
    ...
    {/* The variable T[level][collection[actlevel]-1] is part */
     ...
     if(notfound) go_back(); /* Go back just one level. */
     else
       {/* Add the variable T[level][collection[actlevel]-1]      */   
...
{posint i;
 ...
 for(i=actlevel;i<maxlevel;i++)
   if(cut_is_empty(nextlevel[i])) return FALSE; 
...
{ posint position;     
  for(position=0;position<NV[terme];position++)
    if(startlevel[T[terme][position]]) return FALSE; 
  return TRUE;
}      
...
{ posint position,actterm,maximum,newvariable;
  ...
  for (position=0;position<NV[level];position++)
    {actterm=lock[T[level][position]];  
     ...
     if(maximum<actterm)
       {/* Found a term with a higher number. */
        maximum=actterm;
  ...
  for (;actlevel>maximum;actlevel--) clear_level();                    
  if(islocked)
    {/* Take a new variable (the locked one) from the         */
     /* level-th term.                                        */
     position=collection[actlevel];
     level=nextlevel[actlevel];
...
{ posint position,variable,level;
  ...
  while ((actlevel>0)&&no_stop)
    if(collection[actlevel]==0) 
      ...
      {level=nextlevel[actlevel];
       if(collection[actlevel]==NV[level])
         {/* No new variable to be found in the level-th term. */
...
{posint position,level;
 if(collection[actlevel]!=0)
   {/* A variable has been taken from the nextlevel[actlevel]-th term. */
    level=nextlevel[actlevel];
...
{ posint i;
  ...
  for(i=start[var];i<start[var+1];i++,maxlevel++) nextlevel[maxlevel]=i;
}      
...
{ posint i,j,k,l,m;
  ...
  i=NECESSARY;
  while (l!=0)
    {if(testelem((i-NECESSARY),solution->grset))
       {/* Found a grid that is an element of the solution. */
        l--;
        for(j=1; j<=result->sno; j++)
          {start=fI; k=0;
           /* First try if essential grids are to be */
           ...
           while ((k<GITTER) && (m!=0))
             {if(testelem((k-NECESSARY),solution->grset))
                {m--;
                 if(grid_is_covered(delta,start->info,(j-1),i))
                   connect(result->delta,j,old2new[(i-NECESSARY)],old2new[(k-NECESSARY)]);
                }
              start=start->next;  /* Next grid. */
              k++;                     
             }
...
{ posint i,j=help->ncol;
  if(j>delta[letter][gridno]->ngrset) return FALSE;         
  for(i=0; j!=0; i++)
    if(testelem(i,help->col))
      {j--;
       if (!testelem(i,delta[letter][gridno]->grset))return FALSE;  
