...
{ stacknode hlp;
	if (freestack == freebot) {hlp = newstack();}   /* create new element */
	else { hlp = freestack; freestack = hlp->next;} /* use old element */
	hlp->info = q;
...
{ stacknode hlp;
	...
	hlp->next = freestack; freestack = hlp; /* put hlp on top of freestack*/
 ...
 */
 treenode father,rebal,mark,act;
  ...
  if(cmp(q,rebal) == SMALL) 
    {mark = rebal->tl; newbal = 2;}
  else 
    {mark = rebal->tr; newbal = 1;}
  act = mark;
  for(t=cmp(act,q); t != EQUAL; t=cmp(act,q)) 
    if(t == SMALL)
      {act->bal = 1; act = act->tr;}
    else 
      {act->bal = 2; act = act->tl;}
  ...
  if(!rebal->bal)                /* tree has grown */
    ...
    {if(mark->bal == newbal)     /* single rotation */
       {act = mark;
	if(newbal == 1)
          {rebal->tr = mark->tl; mark->tl = rebal;} 
        else 
          {rebal->tl = mark->tr; mark->tr = rebal;}
	rebal->bal = 0; 
        mark->bal = 0;
       } 
     else                        /* double rotation */
       {if(newbal == 1) 
          {act = mark->tl; mark->tl = act->tr; act->tr = mark;
	   rebal->tr = act->tl; act->tl = rebal;
          } 
        else 
          {act = mark->tr; mark->tr = act->tl; act->tl = mark;
	   rebal->tl = act->tr; act->tr = rebal;
	  }
        rebal->bal = 0; 
        mark->bal = 0;
        if (act->bal == newbal)     rebal->bal = 3 - newbal;
        if (act->bal == 3 - newbal) mark->bal = newbal;
        act->bal = 0;
       }
     /* finishing touch */
     if (father->tr == rebal) father->tr = act;
     else father->tl = act;
    } 
  return FALSE;
} /* search */
...
/* computes the sets of nfa-states which form the dfa-states */ 
{
 ...
 for(q=0; q <= na->qno; q++)
   if(isinit(na->infin[q]))
     {setelem(q,actq);
      if (isfinal(na->infin[q])) actq->final = TRUE; /* init is final */
     }
 push(actq);
         ...
         if(testelem(q,actq))                /* loop: q in actq */
           for(p=0; p <= na->qno; p++)       /* union */
             if(testcon(na->delta,l,q,p))    /* p reached */
               {setelem(p,nxtq);
                if (isfinal(na->infin[p])) nxtq->final = TRUE;
               }
       /* third step */
...
static void cpinfo(dfa da) {
	...
	{while (actq != NULL)  /* go left first (depth first) */
	    {push(actq);actq = actq->tl;}
	if(bottom != top)     /* stack is nonempty */
	    {actq = pop();      /* visit actq */
	    if(with) new2old[actq->number] = actq->dfastate;
  ...
  boole empty=TRUE; /* test: empty set of initial states */
	...
	da->minimal = FALSE;
        ...
        for(q=0;(q<=na->qno)&&empty;q++)
           if(isinit(na->infin[q]))empty=FALSE;
	
        if(empty) da->qno=0;
        else da->qno = compnewstates(na);
