...
{ register posint k;
  ...
  for(j=0; j<=q; j++) 
    {for(k=0; k<=q; k++)
       if (testcon(na->delta,0,j,k)) mat[0][j][k] = TRUE;
       else mat[0][j][k] = FALSE;
     mat[0][j][j] = TRUE;
    }
  ...
  for(i=0; i<=q; i++) 
    {for(j=0; j<=q; j++)
       for(k=0; k<=q; k++) 
         if (rrobin) kleene(1,0,i,j,k);
	 else kleene(0,1,i,j,k);
       rrobin = !rrobin;
     }
...
nfa enfa2nfa(nfa epsa) { 
  ...
  if(epsa->is_eps) 
    {/* construct the epsilon-closure of all states
     ...
     for(state=0; state<=result->qno; state++) 
       if(isinit(result->infin[state])&&(!isfinal(result->infin[state])))
         {for(goal=0; goal<=result->qno; goal++) 
	      ...
	      {setfinalT(result->infin[state]); break;}
         }
