...
#include "test/testUnary.h" /* deadsearch */
...
nfa starnfa(nfa na,boole plus)  { 
  ...
  if(plus) 
    {/* search for a state which is initial and final 
      * in this case L* = L+ 
...
nfa revnfa(nfa na) {
 nfa result;
  ...
  for(i=0; i <= result->qno; i++)
    {if(isfinal(na->infin[i]))setinit(result->infin[i]);
...
dfa minL(dfa indfa) { 
  ...
  for(i=0;i<=indfa->qno;i++)
    if(!indfa->final[i])old2new[i]=++count;  
...
dfa maxL(dfa indfa) { 
  ...
  if(!neg) return simpledfa(FALSE,indfa->sno);
...
{ dfa outdfa;
  ...
  outdfa = newdfa();
  deadsearch(indfa,&q1,&dead,&pos,&neg);
  if(!neg) return simpledfa(TRUE,indfa->sno);
  outdfa->qno = indfa->qno;
  outdfa->sno = indfa->sno;
  outdfa->minimal = FALSE;
  outdfa->init = indfa->init;
