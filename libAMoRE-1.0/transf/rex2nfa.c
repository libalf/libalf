...
static boole complast(mrkfin buf) { 
      ...
      op = rex[lastrex--];
      switch(op) {
      case unionch: epslbl2 = complast(buf);
            epslbl1 = complast(buf);
            prerex[--lastprerex] = op;
            return(epslbl1 || epslbl2);
      case plusch:  epslbl1 = complast(buf);
            prerex[--lastprerex] = op;
            return(epslbl1);
      case starch:  (void)complast(buf);
      case epsch:   prerex[--lastprerex] = op;
            return(TRUE);
      case 'A':
      case 'a':     if (buf != NULL) {
      ...
      case langch:  prerex[--lastprerex] = op;
            return(FALSE);
      case concatch: /* scan e2 first and if eps not in e2 skip over e1 */
            if (complast(buf)) epslbl1 = complast(buf);
            else { (void)complast((mrkfin)NULL); epslbl1 = FALSE; }
            prerex[--lastprerex] = op;
            return(epslbl1);
      default: errex(SWERR,"complast");
      }
      /* NOTREACHED */
} /* complast */
...
static boole compfirst(ndelta buf) { 
      ...
      case skipch:  firstprerex = skipval[firstprerex-1];
      case unskipch:return(compfirst(buf));
      case unionch: epslbl2 = compfirst(buf);
            epslbl1 = compfirst(buf);
            return(epslbl1 || epslbl2);
      case plusch:  epslbl1 = compfirst(buf);
            return(epslbl1);
      case starch:  (void)compfirst(buf);
      case epsch:   return(TRUE);
      case 'A': if (buf != NULL)
            for(hlp=1; hlp <= sno; hlp++)
                  connect(buf,hlp,constate,rposch[firstprerex-1]);
      return(FALSE);
      case 'a': if (buf != NULL)  /* modified by oma, Jan.96 */
            connect(buf,rposlt[firstprerex-1],constate,
                    rposch[firstprerex-1]); 
      return(FALSE);
      case langch:  return(FALSE);
      case concatch:/* skip first(e2) iff eps not in e1 */
            if (compfirst(buf)) return(compfirst(buf));
            else { (void)compfirst((ndelta)NULL); return(FALSE); };
      }
      /* NOTREACHED */
      return(FALSE);
} /* compfirst */
...
static void getexpr(copy) { 
      ...
      case unionch:
      case concatch: getexpr(copy);
      case plusch:
      case starch:   getexpr(copy);
            if (copy) prerex[--getprerex] = op;
            else length += 2;
            break;
      case 'A':
      case 'a':  /* oma */
...
static void compff(posint f, ndelta buf) { 
      ...
      case unionch:	/* call compff two times starting at f */
            prerex[--ffprerex] = skipch; skippos = ffprerex;
            skipval[skippos] = f;
            compff(f,buf);
            compff(f,buf);
      ...
      case plusch:	
            ...
            while ((rex[ffrex]==starch) || (rex[ffrex]==plusch)) {
                  if (rex[ffrex--] == starch) ch = starch;
                  
            }
            prerex[--ffprerex] = skipch; skippos = ffprerex;
            skipval[skippos] = f;
            /* get the expression under the star */
            getrex = ffrex; getprerex = ffprerex;
            getexpr(TRUE);
            ffprerex = getprerex;
      ...
      case 'a':       
            ...
            break;
      case epsch:
      case langch:	prerex[--ffprerex] = ch;
            break;
      default: errex(SWERR,"compff");
      }
} /* compff */
...
nfa rex2nfa(regex re) {
	...
	for(i=0; i<re->erexl; i++) {
              ...
              case starch:
                    ...
                    getrex = rexlen-1;
                    getexpr(FALSE);
                    while ((re->exprex[i+1] == plusch) || 
                           (re->exprex[i+1] == starch)) {
                          i++; 
                    }
                    length++;break;
