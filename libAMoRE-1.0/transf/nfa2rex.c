...
static void init(nfa na) {
	...
	for(i=0; i <= na->qno; i++) {
		...
		if (testcon(na->delta,k,i,j)) {
		    if (!alph) {
			str[0] = itoc[k];
			if (!first) str[1] = unionch;
			else str[1] = '\0';
		    } else {
			strno = strcpy(strno,pi2a(k));
			if (!first) strno = strcat(strno,strunion);
		    }
		    first = FALSE;
		    hlp = ent; l += (posint)strlen(str);
		    CCC(ent,l+1,hlp,str);
		}
	    area[i][j].l = l;
	    if (l) {
	...
	if(isinit(na->infin[i]))
	    ...
	    area[na->qno+1][i].l = 1;
	    if(isfinal(na->infin[i]))eps=TRUE; 
	    }
...
static void destroyloops() {
	...
	if (test(area[i][i].s))       /* @* == @, and @.re2 == re2 */
	    for(j=qst;j <= qno; j++) if (area[i][j].l) {
...
static boole deletenode() {
 posint i,j,l1,l2,l3,w1,w2,w3,delnod;
  string str;
	delnod = qst++;
	for(i=qst; i <= qno ; i++) if (area[i][delnod].l) {		/* re1 != "" */
		l1 = area[i][delnod].l;
	   	if (test(area[i][delnod].s)) w1 = 2; else w1 = 1;

		for(j=qst; j <= qno; j++) if (area[delnod][j].l){	/* re2 != "" */
			l2 = area[delnod][j].l;
	   		if (test(area[delnod][j].s)) w2 = 2; else w2 = 1;
			
			l3 = area[i][j].l;
			if (area[i][j].l) {
	   			if (test(area[i][j].s)) w3 = 2; else w3 = 1;
			} else w3 = 0;
			switch (9*w1 + 3*w2 + w3) {
...
regex nfa2rex(nfa na) {
    ...
    do destroyloops(); while (deletenode());
    destroyloops();
	...
	re->erexl = 1; re->exprex = emptysetstr;
	re->rexl = 1; re->rex = emptysetstr;
    }
