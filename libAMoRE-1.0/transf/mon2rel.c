...
typedef struct reltrees{posint left;     /* left side of relation = element of monoid */
...
{posint pre1,pre2;
 ...
 if(rel1->gen<rel2->gen) return(2);
 else if(rel1->gen>rel2->gen) return(0);
 pre1=rel1->left;
 pre2=rel2->left;
 ...
 if(!pre2) return(1);
 if(last[pre1]<last[pre2]) return(2);
 else                      return(0);
}
...
{posint j;
 ...
 if(emptytree) /* init tree */
   ...
   {search=relroot;
    for(;;)
      {j=revlex(rel,search);
       if (j==1) return(FALSE); /* new relation is an implication of an old one */
       else if(j==0) /* search at left successor  */
         {if(search->lsucc!=NULL) search=search->lsucc; /* continue */
	  else    /* new relation */
	    {inprel=newreltree();
         ...
         {if(search->rsucc!=NULL) search=search->rsucc;  /* continue */
	  else   /* new relation */
	    {inprel=newreltree();
	     inprel->left=rel->left;
	     inprel->gen=rel->gen;
...
{boole new;
 ...
 for(i=0;i<stamon->mno;i++)
   for(gen=1;gen<=stamon->gno;gen++)
     {new=FALSE;
      if(succ[succ[i][gen]][0]!=i) new=TRUE; /* pred(i*gen)!= i */
      else if(stamon->lastletter[succ[i][gen]] != gen) new=TRUE; /* i*gen=i*gen' with gen'< gen */ 
      if(new)
         {rel->gen=gen;
          rel->left=i;
          if(zero==succ[i][gen]) 
            {if((rel->left==zero)||(rel->gen==zero))new=FALSE; /*left=zero,gen=zero*/
             else if(revlex(rel,relfirst)==1)new=FALSE;        /*left*gen=gen'*zero*/
            }
          if(new)
            if(inrel(rel))
