 ...
 while((run<counter)&&(!found))
  {help=queue[run];
   for(gno=0;gno<stamon->gno;gno++)
     {test=c2c[help][gno];
      if(test==stamon->mno) break; /* advance in queue */
      else if(test==no2)  /* no2 found */
	     {found=TRUE;break;}
           else if(!mark[test])
	          {queue[counter++]=test;mark[test]=TRUE;}
     }
   run++;
  }
 for(run=0;run<counter;run++)mark[queue[run]]=FALSE; /* clear mark */
 return(found);  
}
 ...
 if(!dcomplete[dno])
  {dcomplete[dno]=TRUE;
   for(i=1;i<=stamon->ds->dno;i++)
	...
	{if(!dcomplete[i])d2dcomplete(i,dcomplete);
	 for(j=1;j<=stamon->ds->dno;j++)
	     if(d2d[i][j]==1)d2d[dno][j]=1;
	}
  }
}
...
{posint count=0,gen1,gen2,n,help,help1,class,i;
 ...
 if(m==0)return;        /* inversphi(1) computed in output */
 /* compute u */
 class=melem2rno[m];
       ...
       {help=stamon->gensucc[n][gen1];             /* help=n*gen1 */
        if(melem2rno[help]==class)                 /* n*gen1 in rclass of m */
          if((melem2rang[n]>melem2rang[m])||(!(testc2c(class,melem2rno[n],r2r))))
	       ...
	       {stak[high++]=n;stastarf->computed[n]=TRUE;}
             result[count++]=n;
             result[count++]=gen1;
            }
       } /* end for gen1 */
 stastarf->u[m]=newar(count);
 for(i=0;i<count;i++)stastarf->u[m][i]=result[i];
 ...
 for(i=0;i<count;i++)stastarf->v[m][i]=result[i];
 ...
 for(i=0;i<count;i++)stastarf->w0[m][i]=result[i];
 ...
 for(i=0;i<count;i++)stastarf->w1[m][i]=result[i];
 /*tick();*/
} 
...
/* computation of all m which assign a final state to the initial state */
  ...
  for(i=0;i<stamon->mno;i++)
	 if(indfa->final[stamon->no2trans[i][help]])
           {stak[high++]=i;stastarf->computed[i]=TRUE;}
  while(high)
    {high--;inversphi(stak[high]);}
  freebuf();
  return(stastarf);
}
