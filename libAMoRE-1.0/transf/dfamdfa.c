 ...
 if(!(high==(stadfa->qno+1)))
   {
    ...
    if(freeaut) {freedfa(stadfa); dispose(stadfa);}
    stadfa=outdfa;
    d=fdelta;
    statesdeleted=TRUE;
   }
...
/* TRUE iff dfa has only final states or only nonfinal states */
{
 ...
 /* init blocks O(|Q|) */
 last=newarray(2);
 if(with) inblock=newar(numberofstates);
 else inblock=newarray(numberofstates);
 ...
 if(simple)            /* only one state */
    {
     ...
     if(with)
       for(state=0;state<=stadfa->qno;state++)
          inblock[state]=0;
     if(freeaut||(statesdeleted)){freedfa(stadfa); dispose(stadfa);}
    }
 else 
    if(numberofblocks==stadfa->qno) /* no changes */
         {
          if(with)
            for(state=0;state<=stadfa->qno;state++)
              inblock[state]=state;
...
static void mindfa()
{
 ...
 posint bl,j;
 d=stadfa->delta;  
 if(!with) delstates();                               /* delete non-reachable states */
 if(stadfa->qno==0){stadfa->minimal=TRUE;outdfa=stadfa;return;}
    ...
    while(sizeofwaiting)                                        /* 3 */
       {
		...
		numberofblocks++; /* tick();*/                   /* 7 */
               split(bl);                                       /*9,10 */ 
               for(letter=1;letter<=stadfa->sno;letter++) 
                 if(inwaiting[bl][letter]) putinwaiting(letter,numberofblocks);
                    else
                      if(blocksize[bl]>=blocksize[numberofblocks]) putinwaiting(letter,numberofblocks);
                      else putinwaiting(letter,bl);
              }
