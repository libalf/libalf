...
monoid mon;
{
	...
	while(run < mon->ds->dno)
		if(mon->ds->dclassarray[run++]->hsize != 1)
			return (FALSE);
	return (TRUE);
}
...
boole sftest(monoid mon)
{
	if(mon->dclassiscomputed)
		return (test_htrivial(mon));
	else
		return (test_groupfree(mon));
}
...
posint e;
{
		...
		/* test eSe idempotent and commutative */
	{
		for (j = 0; j < run; j++)
			...
			if(!testidem[eSeplace[j]])
				return (2);
		/* test commutative */
		for (j = 0; j < run; j++)
			for (i = j + 1; i < run; i++)
				if(!(mult(stamon, eSeplace[i], eSeplace[j]) == mult(stamon, eSeplace[j], eSeplace[i])))
					return (2);
...
void proptest(language lan, monoid mon)
{
	...
	{			/* all flags are FALSE */
		 ...
		 */
		for (i = 1; i <= stamon->gno; i++)
			if(stamon->gensucc[i][i] != i)
				return;
		for (i = 1; i <= stamon->gno; i++)
			for (j = i + 1; j <= stamon->gno; j++)
				if(stamon->gensucc[i][j] != stamon->gensucc[j][i])
					return;
		lan->localtest = TRUE;
		return;
	}
	...
	if(idemcount >= 2)
		lan->nilpotent = FALSE;
	else if(!(testrightzero(idem[0]) && testleftzero(idem[0])))
		lan->nilpotent = FALSE;
	if(!lan->nilpotent) {
		halfzero = newb_array(idemcount);
		/* halfzero[i] <=> idem[i] is a right or left zero */
		for (i = 0; i < idemcount; i++) {
			if(testrightzero(idem[i]))
				halfzero[i] = TRUE;
			else
				lan->definite = FALSE;
			if(testleftzero(idem[i]))
				halfzero[i] = TRUE;
			else
				lan->revdefinite = FALSE;
		}
...
boole testdd1(monoid mon)
{
	...
	while(faktor) {
		...
		for (i = 0; i < stamon->mno; i++)
			help[i] = mult(stamon, help[i], help[i]);
		faktor = faktor / 2;
	}
	/* test condition 
	 *         (e*i*f*j)^g * e*i*f*l*e * (k*f*l*e)^g
