...
{posint letter,stateno1,stateno2,state1,state2;
 ...
 stateno1=dfa1->qno+1;
 stateno2=dfa2->qno+1;
 if(stateno1!=stateno2)return(FALSE);
 ...
 while(equal && height)
   {
    ...
    for(letter=1;equal&&(letter<=dfa1->sno);letter++) 
      {state1=dfa1->delta[letter][stateno1]; /*abbreviation*/
       ...
       if(mark1[state1])
         {if((!mark2[state2])||(bijection[state1]!=state2))equal=FALSE;}
       else
         if(mark2[state2])equal=FALSE;
         else
           {mark1[state1]=TRUE;
            mark2[state2]=TRUE;  
 ...
 if(equal) /* bijection found, test for final states */
   for(state1=0;equal&&(state1<=dfa1->qno);state1++)
     {state2=bijection[state1]; 
      if(dfa1->final[state1]&&(!dfa2->final[state2]))equal=FALSE;
      if(dfa2->final[state2]&&(!dfa1->final[state1]))equal=FALSE;
     }
 freebuf();
 return(equal);
} /* equiv */
     ...
     for(letter=1;letter<=dfa1->sno;letter++)
       {
        ...
        if(inclusion)
           {if(dfa1->final[r1]&&(!dfa2->final[r2])) result=FALSE;}
        else
           {if(dfa1->final[r1]&&dfa2->final[r2]) result=FALSE;}
