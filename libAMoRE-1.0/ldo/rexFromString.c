...
static string init_idline_alphanum( int maxlet ) {
      ...
      posint IdLineSize; /** size of the identifier line */
...
static PARSE_INPUT init_parseinp(int maxlet, char *s) {
      ...
      */
      ...
      IdLine = ( maxlet > ALPHSIZE ) ? 
          init_idline_alphanum( maxlet )        /* "a0 a1 ..." */
      ...
      for (i = 0; i < 11; ++i) {
            ...
            for (j = 0; j < 11; ++j) precedence[i][j] = prec[i][j];
      }
...
static EXPRESS tree2string(t_elem Root) {
   ...
   while (Node->sons_no != 0) Node = Node->son[0];
   ...
   while (Node != NULL)
   {
      ...
      switch (Node->sons_no)
      {
         case 0:  if ((FirstLet >= 'A') && (FirstLet <= MAXLET))
         ...
         case 1:  switch (FirstLet)
                  ...
                  Node = Node->father;
                  break;
         case 2:  switch (Node->son_passed)
                  {
                     case 0:  Node->son_passed = 1;
                              Node = Node->son[1];
                              while (Node->sons_no != 0) Node = Node->son[0];                                 break;
                     case 1:  Node->son_passed = 0;
                              ++Length;
                              Node = Node->father;
                              break;
                     default: ;
   ...
   while (Node != NULL)
   {
      ...
      switch (Node->sons_no)
      {
         ...
         case 1:  switch (FirstLet)
                  {
                     case leftbrch: break;                             
                     default:  OutString[Count++] = FirstLet; 
                               break;
                  }
                  Node = Node->father;
                  break;
